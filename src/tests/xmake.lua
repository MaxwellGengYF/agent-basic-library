...
target("test_text_ops_objs")
    set_kind("object")
    add_files("test_text_ops.c")
    add_deps("agent_core", "yyjson", "test_runner")
    set_languages("c11")
    set_warnings("all")
    on_load(function(target)
        apply_msvc_compat(target)
        target:add("includedirs", "src/text_ops/include")
    end)
target_end()
...
target("test_text_ops")
    set_kind("binary")
    add_deps("test_text_ops_objs")
    add_files("test_main.c")
    add_defines('TEST_SUITE_RUNNER=run_text_ops_tests')
    set_languages("c11")
    set_warnings("all")
    on_load(function(target)
        apply_msvc_compat(target)
    end)
    add_tests("test_text_ops", {group = "unit"})
target_end()
